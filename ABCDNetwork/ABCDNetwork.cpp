 ...
 * This project is an A-B-C-D feed-forward neural network that can be both ran and trained using back propagation.
 ...
 *
 * Table of Contents:
 * 1. double sigmoid(double value)
 * 2. double sigmoidDeriv(double value)
 * 3. double linear(double value)
 * 4. double linearDeriv(double value)
 * 5. double randDouble()
 * 6. void printTime(long milliseconds)
 * 7. void setConfigurationParameters()
 * 8. void echoConfigurationParameters()
 * 9. void allocateMemory()
 * 10. void randomizeWeights()
 * 11. void saveWeights()
 * 12. void loadWeights()
 * 13. void populateArrays()
 * 14. void runNetwork()
 * 15. void runForTrain()
 * 16. void trainNetwork()
 * 17. void reportNetworkState()
 * 18. void reportTruthTable()
 * 19. void reportTrainingResults()
 * 20. void reportResults()
 * 21. void runAllTestCases()
...
#define NUM_ACT_LAYERS     4
#define INPUT_LAYER        0
#define FIRST_HIDDEN       1
#define SECOND_HIDDEN      2
...
void setConfigurationParameters()
{
   ...
   layerConfig = new int[NUM_ACT_LAYERS];
...
void echoConfigurationParameters()
{
   ...
   cout << DIVIDER;
   cout << layerConfig[INPUT_LAYER] << "-";
   cout << layerConfig[FIRST_HIDDEN] << "-";
   cout << layerConfig[SECOND_HIDDEN] << "-";
   cout << layerConfig[OUTPUT_LAYER] << " Network\n";
   cout << DIVIDER;
...
void allocateMemory()
{
   ...
   weights = new DARRAY2D[NUM_ACT_LAYERS];

   n = FIRST_HIDDEN;
   weights[n] = new DARRAY1D[layerConfig[n]];
   for (k = 0; k < layerConfig[n]; ++k) weights[n][k] = new double[layerConfig[n - 1]];

   n = SECOND_HIDDEN;
   weights[n] = new DARRAY1D[layerConfig[n]];
   for (j = 0; j < layerConfig[n]; ++j) weights[n][j] = new double[layerConfig[n - 1]];

   n = OUTPUT_LAYER;
   weights[n] = new DARRAY1D[layerConfig[n]];
   for (i = 0; i < layerConfig[n]; ++i) weights[n][i] = new double[layerConfig[n - 1]];
   ...
   if (isTraining)
   {
      thetas = new DARRAY1D[NUM_ACT_LAYERS];
      psis = new DARRAY1D[NUM_ACT_LAYERS];
...
void randomizeWeights()
{
   ...
   {
      for (m = 0; m < layerConfig[n - 1]; ++m)
      {
         weights[n][k][m] = randDouble();
      }
   } // for (k = 0; k < layerConfig[n]; ++k)
...
void runNetwork()
{
   ...
   for (k = 0; k < layerConfig[n]; ++k)
   {
      ...
      for (m = 0; m < layerConfig[n - 1]; ++m)
      {
         theta += a[n - 1][m] * weights[n][k][m];      // weighted sum of the previous layer's activations
      }

      a[n][k] = actFunct(theta);
   } // for (k = 0; k < layerConfig[n]; ++k)
   ...
   for (i = 0; i < layerConfig[n]; ++i)
   {
      ...
      for (j = 0; j < layerConfig[n - 1]; ++j)
      {
         theta += a[n - 1][j] * weights[n][i][j];      // weighted sum of the previous layer's activations
      }

      a[n][i] = actFunct(theta);
   } // for (i = 0; i < layerConfig[n]; ++i)
...
void runForTrain()
{
   ...
   for (k = 0; k < layerConfig[n]; ++k)
   {
      ...
      for (m = 0; m < layerConfig[n - 1]; ++m)
      {
         thetas[n][k] += a[n - 1][m] * weights[n][k][m];
      }

      a[n][k] = actFunct(thetas[n][k]);
   } // for (k = 0; k < layerConfig[n]; ++k)
...
void trainNetwork()
{
   ...
   while (avgError > avgErrCut && curIterations < maxIterations)           // trains the network
   {
      ...
      for (curTrainingCase = 0; curTrainingCase < numTrainingSets; ++curTrainingCase)
      {
         ...
         n = SECOND_HIDDEN;                              // between the output and second hidden activation layer
         for (j = 0; j < layerConfig[n]; ++j)
         {
            omega = 0.0;
            for (i = 0; i < layerConfig[n + 1]; ++i)
            {
               omega += psis[n + 1][i] * weights[n + 1][i][j];
               weights[n + 1][i][j] += lambda * psis[n + 1][i] * a[n][j];
            }

            psis[n][j] = omega * actFunctDeriv(thetas[n][j]);
         } // for (j = 0; j < layerConfig[n]; ++j)

         n = FIRST_HIDDEN;                              // between the second hidden and first hidden activation layer
         for (k = 0; k < layerConfig[n]; ++k)
         {
            omega = 0.0;
            for (j = 0; j < layerConfig[n + 1]; ++j)
            {
               omega += psis[n + 1][j] * weights[n + 1][j][k];
               weights[n + 1][j][k] += lambda * psis[n + 1][j] * a[n][k];
            }

            psis[n][k] = omega * actFunctDeriv(thetas[n][k]);

            for (m = 0; m < layerConfig[n - 1]; ++m)
            {
               weights[n][k][m] += lambda * psis[n][k] * a[n - 1][m];
            }
         } // for (k = 0; k < layerConfig[n]; ++k)
